...
def create_dielek(num_dielek):
    ...
    for i in range(num_dielek):
        questions["dielectric"].append(
            "What are the parameters of the {}-th dielectric object?".format(i + 1)
        )
...
def create_meas(num_meas):
    ...
    for i in range(num_meas):
        questions["measurement"].append(
            "What are the parameters of the {}-th measurement point?".format(i + 1)
        )
        functions["measurement"].append(
            lambda **kwargs: (kwargs["pos_x"], kwargs["pos_y"])
        )
    return None
...
class placing_line_source:
    ...
    def choose_profile(self, profile):
        ...
        if profile == 1:
            ...
            var_description["source_parameters"] = [
                "Source amplitude (J₀) [A]",
                "Pulse width (σ) [s]",
                "Central time (tc) [s]",
            ]
            questions[
            ...
            var_description["source_parameters"] = [
                ...
                "Pulse width (σ) [s]",
                "Central time (tc) [s]",
                "Central frequency (ω_c) [Hz]",
            ]
            questions[
                "source_parameters"
            ] = "What are the parameters for the Gaussian-modulated sinusoidal radio-frequency pulse source?"
            functions["source_parameters"] = [self.pulse]
        else:
    ...
    def pulse(self, **kwargs):
        ...
        print()
        print(
            "It is recommended to take the discretization steps between exact {l1} & {l2} [m]".format(
                l1=lambda_min / 30, l2=lambda_min / 20
            )
        )
        print(
            "Human readable: {mini:.2g} < Δspace < {maxi:.2g}".format(
                mini=lambda_min / 30, maxi=lambda_min / 20
            )
        )
        return source_obj
...
def discretization(**kwargs):
    max_d_t = 1 / (
        c * np.sqrt(1 / (kwargs["Delta_x"] ** 2) + 1 / (kwargs["Delta_y"] ** 2))
    )
    print(
        "The time discretization should be less than {} [s] for a stable simulation.".format(
            max_d_t
        )
    )
    return kwargs
...
def dialog():
    ...
    var_description["space"] = [
        "The length of the simulation domain (x-axis) [m]",
        "The height of the simulation domain (y-axis) [m]",
    ...
    var_description["eps_avg"] = [
        "Relative permittivity averaging, enter number of choice"
    ]
    questions[
        "eps_avg"
...
2) Gaussian-modulated sinusoidal radio-frequency pulse"""
    ...
    var_description["discretization_space"] = [
        "Step in x-direction (Δx) [m]",
        "Step in y-direction (Δy) [m]",
    ]
    ...
    var_description["visualize_fields"] = [
        "Seconds of the visualisation time step or 0"
    ]
    questions[
        "visualize_fields"
...
x) Every x seconds, with x a value given by you and between 0 and the length of simulation"""
    functions["visualize_fields"] = [
        lambda visualize_fields: visualize_fields
        // simulation["discretization_time"]["Delta_t"]
    ]
    ...
    for k in keys:
        ...
        for i, qst in enumerate(subqst):
            var_name_desc = {
                name: desc for name, desc in zip(var_names[k], var_description[k])
            }
...
def simulate(simulation):
    ...
    box.define_discretization(
        **simulation["discretization_space"], **simulation["discretization_time"]
    )
    titles = [
        "  at ({:g} , {:g})".format(meas[0], meas[1])
        for meas in simulation["measurement"]
    ]
    box.add_measurement_points(simulation["measurement"], titles)
    measurements = box.FDTD(
        eps_averaging=simulation["eps_avg"],
        plot_space=True,
        visualize_fields=simulation["visualize_fields"],
    )
    measurement.plot(
        measurements[0].time_E,
        box.source.get_current(measurements[0].time_E),
        "time [s]",
        "current [A]",
        "Source current",
        filename="current",
